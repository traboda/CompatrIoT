 ...
 ******************************************************************************
 * @file           : usbd_cdc_if.c
 * @version        : NEFASTOR.COM Simple UART API implementation based on
 *v2.0_Cube
 ...
 * therefore considered to be provided AS-IS. That code is copyright
 *STMicroelectronics.
 *
 * The code added by Jean ROCH for the purpose of creating a practical UART API
 * is Copyright (c) 2022 Jean Roch.
 *
 * That code is free software: you can redistribute it and/or modify it under
 *the terms of the GNU General Public License as published by the Free Software
 *Foundation, either version 3 of the License, or (at your option) any later
 *version.
 *
 * This CDC library is distributed in the hope that it will be useful, but
...
static int8_t CDC_Init_FS(void)
{
  ...
  cdc_tx_fifo.data = UserTxBufferFS; // Use the buffer generated by Cube
  cdc_tx_fifo.wr = 0;
  cdc_tx_fifo.rd = 0;
  cdc_tx_fifo.lb = 0;
  cdc_rx_fifo.data = UserRxBufferFS; // Use the buffer generated by Cube
  cdc_rx_fifo.wr = 0;
  cdc_rx_fifo.rd = 0;
  cdc_rx_fifo.lb = 0;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  if (cdc_rx_fifo.wr >= RX_BUFFER_MAX_WRITE_INDEX) {
    // Solution : wrap-around (and save wr as lb)
    cdc_rx_fifo.lb = cdc_rx_fifo.wr;
    cdc_rx_fifo.wr = 0;
  }
  // Tell the driver where to write the next incoming packet
  USBD_CDC_SetRxBuffer(&hUsbDeviceFS, cdc_rx_fifo.data + cdc_rx_fifo.wr);
...
void cdc_init() {
  while (cdc_tx_fifo.data == 0)
    ; // Wait until the CDC library calls CDC_Init_FS, by checking for a
      // non-null buffer pointer
}
void cdc_putchar(uint8_t ch) { cdc_send(&ch, 1); }
void cdc_putnchar(uint8_t ch, uint16_t count) {
	while(count--)
	cdc_send(&ch, 1);
}

void cdc_getchar(uint8_t *ch) { cdc_recv(ch, 1); }

int cdc_send(uint8_t *buf, uint16_t len) {
  // Step 1 : calculate the occupied space in the Tx FIFO
  int cap = cdc_tx_fifo.wr - cdc_tx_fifo.rd; // occupied capacity
  ...
  int tail = APP_TX_DATA_SIZE - cdc_tx_fifo.wr;
  if (tail >= len) {
    // Copy buf into the tail of the FIFO
    memcpy(&cdc_tx_fifo.data[cdc_tx_fifo.wr], buf, len);
    // Update "wr" index
    cdc_tx_fifo.wr += len;
    // In case "len" == "tail", next write goes to the head
    if (cdc_tx_fifo.wr == APP_TX_DATA_SIZE)
      cdc_tx_fifo.wr = 0;
  } else {
    // Copy the head of "buf" to the tail of the FIFO
    memcpy(&cdc_tx_fifo.data[cdc_tx_fifo.wr], buf, tail);
    // Copy the tail of "buf" to the head of the FIFO :
    memcpy(cdc_tx_fifo.data, &buf[tail], len - tail);
    // Update the "wr" index
    cdc_tx_fifo.wr = len - tail;
  }
...
int cdc_recv(uint8_t *buf, uint16_t len) {
  ...
  int cap = cdc_rx_fifo.wr - cdc_rx_fifo.rd;
  if (cap == 0)
    return 0;              // Empty FIFO, no data to read
  if (cap < 0)             // FIFO contents wrap around
    cap += cdc_rx_fifo.lb; // Notice the use of lb
  ...
  while (len) {
    len--;
    *buf = cdc_rx_fifo.data[cdc_rx_fifo.rd];
    buf++;
    cdc_rx_fifo.rd++;                     // Update read index
    if (cdc_rx_fifo.rd == cdc_rx_fifo.lb) // Check for wrap-around
      cdc_rx_fifo.rd = 0;                 // Follow wrap-around
  }
  return retval;
}
...
void cdc_service() {
  ...
  if (hcdc->TxState == 0) {
    ...
    cdc_tx_fifo.rd = cdc_tx_fifo.lb;
    // Compute how much data is in the FIFO
    int cap = cdc_tx_fifo.wr - cdc_tx_fifo.rd;
    if (cap != 0) // The FIFO is empty : return immediately
    {
      if (cap < 0) // The FIFO contents wrap-around
      {
        // Send only the tail of the FIFO
        USBD_CDC_SetTxBuffer(&hUsbDeviceFS, &cdc_tx_fifo.data[cdc_tx_fifo.rd],
                             APP_TX_DATA_SIZE - cdc_tx_fifo.rd);
        USBD_CDC_TransmitPacket(&hUsbDeviceFS);
        cdc_tx_fifo.lb = 0; // Lock the tail’s data
      } else                // No wrap-around : send the whole FIFO
      {
        USBD_CDC_SetTxBuffer(&hUsbDeviceFS, &cdc_tx_fifo.data[cdc_tx_fifo.rd],
                             cap);
        USBD_CDC_TransmitPacket(&hUsbDeviceFS);
        cdc_tx_fifo.lb = cdc_tx_fifo.wr; // lock the data
      }
    }
  }
}
